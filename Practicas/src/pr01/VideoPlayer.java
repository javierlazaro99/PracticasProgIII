...
public class VideoPlayer extends JFrame {
	...
	public VideoPlayer() {
		...
		bAnyadir.addActionListener( new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				ficheros = JOptionPane.showInputDialog(miVentana, "Introduzca el nombre del fichero", "Añadir ficheros");
	...
	private void lanzaVideo() {
		if (mediaPlayerComponent.getMediaPlayer()!=null &&
			...
			//Sacar la fecha al iniciar el video
			long fecha = ficVideo.lastModified();
			Date d = new Date(fecha);
			lMensaje.setText(f.format(d));
			lMensaje.repaint();
	...
	private static File pedirCarpeta() {
		JFileChooser fcCarpeta = new JFileChooser();
		fcCarpeta.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
		
		int seleccion = fcCarpeta.showOpenDialog(miVentana);
		
		if(seleccion == JFileChooser.APPROVE_OPTION) {
			File carpeta = fcCarpeta.getSelectedFile();
			return carpeta;
		}else {
			return null;
		}
	...
	public static void main(String[] args) {
		...
		if (args.length < 2) {
			...
			ficheros = JOptionPane.showInputDialog(miVentana, "Introduce el nombre del fichero", "Añadir ficheros");
